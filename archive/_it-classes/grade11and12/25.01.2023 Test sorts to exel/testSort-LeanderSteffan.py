...
def testbubble(n):
    testListe = createlist(n)
    startm = time.time()
    bubble_sort(testListe)  # Zu testende Methode
    endem = time.time()
...
def testinsertion(n):
    testListe = createlist(n)
    startm = time.time()
    insertionsort(testListe)  # Zu testende Methode
    endem = time.time()
...
def writetocsv(n):
    ...
    string = "merge"
    for i in range(len(n)):
        string += ";" + str(testmerge(n[i]))
    string += "\n"
    file.write(string)
    string = "bubble"
    for i in range(len(n)):
        string += ";" + str(testbubble(n[i]))
    string += "\n"
    file.write(string)
    string = "insertion"
    for i in range(len(n)):
        string += ";" + str(testinsertion(n[i]))
    string += "\n"
    file.write(string)
    file.close()
...
n = input("welche listenlängen sollen getestet werden? mit ' ' trennen (z.B.: 10 100): ")
n = n.split(" ")
for i in range(len(n)):
    n[i] = int(n[i])
writetocsv(n)
