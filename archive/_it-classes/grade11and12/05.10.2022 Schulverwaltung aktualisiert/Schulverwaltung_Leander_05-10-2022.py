class Schulverwaltung:
    ...
    def sucheSchueler(self, name):
        for schueler_o in self.schueler_ol:
            if schueler_o.name==name:
                return schueler_o
    def sucheLehrer(self, name):
        for lehrer_o in self.lehrer_ol:
            if lehrer_o.name==name:
                return lehrer_o
    def neuerKurs(self, name, typ, lehrer_o, raum_o):
        self.kurse_ol.append(Kurs(name, typ, lehrer_o, raum_o))
    def sucheRaum(self, nr):
        for raum_o in self.raeume_ol:
            if raum_o.nr==nr:
                return raum_o
...
a = schulverwaltungs_o.sucheLehrer("Einstein")
raum = schulverwaltungs_o.sucheRaum("202")
schulverwaltungs_o.neuerKurs("inf", "LK", a, raum)

print(schulverwaltungs_o.kurse_ol[-1].lehrer_o.vorname) #Vorname des Lehrers, der den zuletzt eingefuegten Kurs unterrichtet
